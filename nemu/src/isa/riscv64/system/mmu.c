...
paddr_t isa_mmu_translate(vaddr_t vaddr, int len, int type) {
  ...
  if (pte2.V == 0 || (pte2.R == 0 && pte2.W == 1)) {
    panic("vaddr: 0x%8lX; pte2: 0x%16lX", vaddr, pte2.val);
  }
  // PA has no such exception: manual II/P76/2
  assert(!(pte2.R || pte2.W || pte2.X)); // No Large Pages in PA

  word_t pte1_addr = (pte2.ppn << 12) + sv39_vaddr.vpn1*sizeof(pte_t);
  pte_t pte1; pte1.val= paddr_read((paddr_t)pte1_addr, sizeof(word_t));
  if (pte1.V == 0 || (pte1.R == 0 && pte1.W == 1)) {
    panic("vaddr: 0x%8lX; pte1: 0x%16lX", vaddr, pte1.val);
  }
  // PA has no such exception: manual II/P76/2
  assert(!(pte1.R || pte1.W || pte1.X)); // No Large Pages in PA

  word_t pte0_addr = (pte1.ppn << 12) + sv39_vaddr.vpn0*sizeof(pte_t);
  pte_t pte0; pte0.val= paddr_read((paddr_t)pte0_addr, sizeof(word_t));
  if (pte0.V == 0 || (pte0.R == 0 && pte0.W == 1))  {
    panic("vaddr: 0x%8lX; pte0: 0x%16lX", vaddr, pte0.val);
  }
  // PA has no such exception: manual II/P76/2
